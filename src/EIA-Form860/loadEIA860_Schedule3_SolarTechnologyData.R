################################################
## Load generator info from EIA Form 860 Schedule 3 - Solar Technology Data
################################################
library(tidyverse)
library(lubridate)
library(readxl)
...
i_am("src/EIA-Form860/loadEIA860_Schedule3_SolarTechnologyData.R")
...
#Form 860 Started collecting solar data in 2013
year.start = max(as.integer(project.config$sources$`EIA-Form860`$`start-year`),2013)
...
read_json(
  here(
    file.path("src","EIA-Form860","EIA860_Schedule3_Solar_Colspec.json")
    )
  ) -> colspec
...
detect_start_end <- function(path,sheet) {
  ...
  #Want an NA if NOTE does not exist
...
for(yr in year.start:year.end) {
  ...
  ##########################
  ##########################
  ## Identify the Schedule 3 Solar file
  ##########################
  ##########################
  ...
  ######################
  if(yr >= 2013) {
    ## 2013 and later - look for files with "Solar" in their name
    filelist <- filelist[str_detect(filelist,"Solar")]
  ...
  if(length(filelist) == 0) {
    stop(str_c("Files exist in source data folder for ",yr," but none match a Form 3 Solar data file name."))
    next
  }
  
  if(length(filelist) > 1) {
    stop(str_c("Multiple files in source data folder for ",yr," match a Form 3 Solar data file name."))
    next
  }
  ...
  for(sheet in sheetlist) {
    ...
    case_when(
      sheet %in% c("Operable") ~ "Operating",
      sheet %in% c("Retired and Canceled") ~ "Retired",
...
numeric.cols = c(
  ...
  "tilt.angle",
  "net.capacity.dc.mw",
  "net.capacity.under.stc",
  "virtual.net.metering.capacity.dc.mw"
)
for(c in numeric.cols) {
  ...
  all.solar %>%
    mutate(!!c := as.numeric(!!rlang::sym(c))) -> all.solar
}

boolean.cols = c(
  "has.lense.mirror",
  "has.single.axis.tracking",
  "has.dual.axis.tracking",
  "has.fixed.tilt",
  "has.fixed.tilt.eastwest",
  "has.parabolic.trough",
  "has.linear.fresnel",
  "has.power.tower",
  "has.dish.engine",
  "has.other.solar.tech",
  ...
  "is.other.material",
  "has.net.metering",
  "has.virtual.net.metering"
)
for(c in boolean.cols) {
  all.solar %>%
    mutate(
      temp.xxx = !!rlang::sym(c),
      temp.xxx = case_when(
        str_to_upper(temp.xxx) %in% c("Y","YES","TRUE","T","1") ~ "TRUE",
        str_to_upper(temp.xxx) %in% c("N","NO","FALSE","F","0") ~ "FALSE",
        str_trim(str_to_upper(temp.xxx)) %in% c("","NA") ~ "NA",
        TRUE ~ temp.xxx
      )
    ) -> all.solar
  
  #Display values that won't recast to a boolean
  all.solar %>%
    filter(!(temp.xxx %in% c("TRUE","FALSE","NA"))) %>%
    drop_na(temp.xxx) %>%
    distinct(temp.xxx) -> bad.vals
  
  if(nrow(bad.vals) > 0) {
    writeLines(str_c("Column ",crayon::bold(c)," should be boolean but contains the following non-numeric values:"))
    print(bad.vals,n=Inf)
  }
...
for(ym in year.month.cols) {
  ...
  all.solar %>%
    ...
    select(-any_of(c(".year",".month"))) -> all.solar
}
...
if("rds" %in% project.local.config$output$formats) {
  ...
  all.solar %>%
    write_rds(
      file.path(path.EIA860.out,"rds","Form860_Schedule3_Solar.rds.gz"),
      compress="gz"
    )
