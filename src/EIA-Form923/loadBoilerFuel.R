############################################################
## load_eia923_boiler_fuel.R
##
## Loads boiler fuel data (which appears to be boiler+generator fuel)
## from EIA-923.
##
...
for(yr in startYear:endYear) {
  ...
  sheet.list.1 <- sheet.list[str_detect(str_to_upper(sheet.list),"BOILER")]
  ...
  read_excel(
    ...
    rename(
      ...
      fuel.code = `REPORTED FUEL TYPE CODE`,
      fuel.units = `PHYSICAL UNIT LABEL`
    ) -> this.year
  ...
  this.year %>%
    select(
      orispl.code,eia.unit.id,fuel.code,fuel.units,
      contains("Month")
    ) %>% 
...
all.data %>%
  ...
  select(
    orispl.code,eia.unit.id, Month, everything()
  ) %>%
  rename(
    fuel.consumed = `QUANTITY OF FUEL CONSUMED`,
    heat.content = `MMBTU PER UNIT`,
    sulfur.content = `SULFUR CONTENT`,
    ash.content = `ASH CONTENT`
  ) %>%
  arrange(orispl.code,eia.unit.id, Month) -> all.data
...
all.data %>%
  write_rds(
    file.path(eia923.path,"data","out",eia923.version,"R","EIA923_Boiler_Fuel.rds.gz"), 
    compress="gz"
    ) %>%
  rename_all(
  ...
  write_dta(
    file.path(eia923.path,"data","out",eia923.version,"Stata","EIA923_Boiler_Fuel.dta")
  )
